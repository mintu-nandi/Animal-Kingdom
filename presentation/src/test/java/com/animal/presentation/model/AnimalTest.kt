...
class AnimalTest {
    ...
    @Test
    fun testLifespan() {
        val data9 = animal.copy(lifespan = "9")
        assertEquals("Lifespan: 9 yrs (Not very long)", data9.lifespan())

        val data14 = animal.copy(lifespan = "14")
        assertEquals("Lifespan: 14 yrs (Kind of average)", data14.lifespan())

        val data23 = animal.copy(lifespan = "23")
        assertEquals("Lifespan: 23 yrs (A long time)", data23.lifespan())
    }

    @Test
    fun testDiet() {
        val data = animal.copy(diet = "Primarily fruit and leaves, some invertebrates")
        assertEquals("Diet: Primarily fruit and leaves, some invertebrates", data.diet())
    }
